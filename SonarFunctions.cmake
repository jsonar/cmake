...
function(set_package_and_file_name_for_component)
  ...
  if(generator STREQUAL "RPM")
    ...
    set (package_file_name
      "${PKG_FILE_NAME}-${CPACK_PACKAGE_VERSION_MAJOR}.${CPACK_PACKAGE_VERSION_MINOR}.${CPACK_PACKAGE_VERSION_PATCH}${CPACK_PACKAGE_VERSION_EXTRA}-1.${RHEL}.${CPACK_RPM_PACKAGE_ARCHITECTURE}")
...
macro(set_alternate_linker linker)
  ...
  if(LINKER_EXECUTABLE)
    message(STATUS "Found linker ${linker} at ${LINKER_EXECUTABLE}")
    if("${CMAKE_CXX_COMPILER_ID}" STREQUAL "Clang" AND "${CMAKE_CXX_COMPILER_VERSION}" VERSION_LESS 12.0.0)
      message(STATUS "Adding linker option -ld-path=${linker}")
      add_link_options("-ld-path=${linker}")
    elseif("${linker}" STREQUAL "mold" AND "${CMAKE_CXX_COMPILER_ID}" STREQUAL "GNU" AND "${CMAKE_CXX_COMPILER_VERSION}" VERSION_LESS 12.1.0)
      # ex: /usr/local/bin/mold -> /usr/local
      cmake_path(GET LINKER_EXECUTABLE PARENT_PATH MOLD_LIBEXEC)
      cmake_path(GET MOLD_LIBEXEC PARENT_PATH MOLD_LIBEXEC)
      # ex: /usr/local -> /usr/local/libexec/mold
      cmake_path(APPEND MOLD_LIBEXEC "libexec/mold")
      if(EXISTS "${MOLD_LIBEXEC}")
        message(STATUS "Adding linker option -B${MOLD_LIBEXEC}")
        add_link_options("-B${MOLD_LIBEXEC}")
      else()
        message(WARNING "Path ${MOLD_LIBEXEC} not found, mold can't be used")
      endif()
    else()
      message(STATUS "Adding linker option -fuse-ld=${linker}")
